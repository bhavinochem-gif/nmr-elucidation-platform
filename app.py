...
st.markdown("Automated 1D/2D NMR processing, JEOL Delta lock calibration, spectral synthesis, and publication-grade reporting.")
...
with st.sidebar:
    ...
    solvent = st.selectbox(
        "Deuterated Solvent (Lock Reference):",
        list(SOLVENT_TABLE.keys()),
        index=0,  # Default: DMSO-d6
        help="Used to calibrate the zero-point frequency from the residual solvent peak."
    )
...
# --- PROTON NMR INGESTION ---
with col_ingest_1h:
    st.markdown("#### 🔹 ¹H NMR Ingestion")
    up_fid_1h = st.file_uploader("Upload ¹H JEOL File (.jdf):", type=["jdf"], key="fid_1h")
    
    if up_fid_1h:
        with st.spinner("Processing ¹H JEOL Spectrum (Solvent Reference Lock & Deconvolution)..."):
        ...
        st.success(f"✅ Extracted {len(exp_1h_peaks)} ¹H resonances (Range: {np.max(exp_1h_ppm):.1f} to {np.min(exp_1h_ppm):.1f} ppm).")
    else:
        h_manual = st.text_input("Manual ¹H Peaks (ppm):", "11.50, 8.55, 7.70, 7.30, 4.60, 3.33, 2.50, 1.25")
        if h_manual.strip():
            exp_1h_peaks = [
...
# --- CARBON NMR INGESTION ---
with col_ingest_13c:
    st.markdown("#### 🔸 ¹³C NMR Ingestion")
    up_fid_13c = st.file_uploader("Upload ¹³C JEOL File (.jdf):", type=["jdf"], key="fid_13c")
    
    if up_fid_13c:
        with st.spinner("Processing ¹³C JEOL Spectrum (Solvent Reference Lock & Peak Picking)..."):
        ...
        c_manual = st.text_input("Manual ¹³C Peaks (ppm):", "170.1, 154.5, 148.5, 138.0, 127.5, 83.5, 48.0, 21.0")
        if c_manual.strip():
            exp_13c_peaks = [{"ppm": float(x.strip())} for x in c_manual.split(",") if x.strip()]

# -----------------------------------------------------------------------------
# 3. ABOVE-AND-BELOW SPECTRA WINDOWS (FULL 16 TO -1 PPM SCALE)
# -----------------------------------------------------------------------------
...
ax_pred.plot(pred_ppm_axis, pred_sim_spec, color="#0B3C5D", lw=1.1, label="In Silico Synthesized Lineshape")
...
ax_pred.set_xlim(full_xlim[0], full_xlim[1])  # Inverted standard scale: 16.0 to -1.0 ppm
ax_pred.set_xlabel("Chemical Shift δ (ppm)", fontweight="bold", fontsize=8.5)
ax_pred.set_ylabel("Abundance (a.u.)", fontweight="bold", fontsize=8.5)
ax_pred.set_title(f"Predicted {target_nuc} Spectrum ({solvent}, {target_freq:.1f} MHz)", fontsize=9.5, fontweight="bold")
...
# --- BOTTOM WINDOW: EXPERIMENTAL JEOL SPECTRUM ---
...
if exp_ppm_show is not None and exp_spec_show is not None:
    ...
    ax_exp.set_xlim(full_xlim[0], full_xlim[1])  # Identical aligned scale: 16.0 to -1.0 ppm
    ax_exp.set_xlabel("Chemical Shift δ (ppm)", fontweight="bold", fontsize=8.5)
    ax_exp.set_ylabel("Abundance (a.u.)", fontweight="bold", fontsize=8.5)
    ax_exp.set_title(f"Experimental {target_nuc} Spectrum ({solvent}, {target_freq:.1f} MHz)", fontsize=9.5, fontweight="bold")
    ...
    dummy_ppm = np.linspace(full_xlim[0], full_xlim[1], 500)
    ax_dummy.plot(dummy_ppm, np.zeros_like(dummy_ppm), color="gray", linestyle="--", alpha=0.5)
    ax_dummy.set_xlim(full_xlim[0], full_xlim[1])
    ax_dummy.set_xlabel("Chemical Shift δ (ppm)", fontweight="bold", fontsize=8.5)
    ax_dummy.set_ylabel("Abundance (a.u.)", fontweight="bold", fontsize=8.5)
    ax_dummy.set_title(f"Experimental {target_nuc} Spectrum (Awaiting File Upload)", fontsize=9, color="gray")
    ax_dummy.text((full_xlim[0] + full_xlim[1]) / 2.0, 0.5, f"Upload a {target_nuc} .jdf file in Section 2 to display", ha="center", va="center", color="gray", fontsize=9)
...
with st.expander("🔬 Quantum Mechanical Spin System Simulator (AB / ABX Systems)", expanded=False):
    ...
    with qc2:
        try:
            ...
            ax_q.set_ylabel("Abundance", fontsize=8)
